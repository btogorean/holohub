        ...
        for key, gxf_tensor in gxf_entity.items():
            try:
                ...
                if tensor.memory_storage_type == holoscan_proto.Tensor.MemoryStorageType.kHost:
                    ...
                    cp_array = cp.asarray(gxf_tensor, dtype = TensorProto.PRIMITIVE_TYPES[tensor.primitive_type])
    ...
    def _proto_tensors_to_gxf_tensors(entity_request, gxf_entity):
        for key, proto_tensor in entity_request.tensors.items():
            try:
                ...
                if proto_tensor.memory_storage_type == holoscan_proto.Tensor.MemoryStorageType.kHost:
                    tensor = as_tensor(
                        np.frombuffer(proto_tensor.data, dtype=dtype).reshape(proto_tensor.dimensions).copy()
                    )
                elif (
                    holoscan_proto.Tensor.MemoryStorageType.kDevice
                    or holoscan_proto.Tensor.MemoryStorageType.kSystem
                    ...
                    tensor = as_tensor(
                        cp.asarray(ProtoTensorCudaArrayInterface(proto_tensor))
                    )
